...
**Note — hollow stationery:** A Designer `.wep` project carries **no** `.base` snapshots, so it has no Level 3 and resolves straight from the installation (Level 4). That is exactly what makes a `.wep` usable directly as a *hollow stationery* by an AutoMap job (`<Project ... useAsStationery="True">`, ePublisher 2026.1+). A standalone Stationery (`.wxsp`) — and an Express `.wrp` — instead bundle `.base` at this level. See the automap skill's `references/job-file-guide.md`.
