...
- `plugins/webworks-agent-skills/skills/reverb2/scripts/resolve-landmarks.py` — the resolver (now extended in v2 with `--remote-base-url` and the cache state machine)
- `plugins/webworks-agent-skills/skills/reverb2/workflows/landmark-resolution.md` — workflow doc (rewritten in v2 to cover both local and remote sources)
- `plugins/webworks-agent-skills/skills/reverb2/tests/verify-landmark-resolver.py` — verification scenarios (21 in v1 → 80 in v2) covering opaque IDs, multi-chunk merge, last-write-wins, and the full CLI contract
- `plugins/webworks-agent-skills/skills/reverb2/tests/fixtures/` — handcrafted `_lx.js` fixtures (single-chunk and multi-chunk)
